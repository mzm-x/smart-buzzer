...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['CONTENT_TYPE']) && strpos($_SERVER['CONTENT_TYPE'], 'application/json') !== false) {
    ...
    if (isset($data['businessName'])) {
        ...
        // Determine package and reviews quantity (Mid-Year Sale packages)
        $packageName = 'Unknown';
        $reviewsQty = '-';
        ...
        if (strpos(strtoupper($action), 'STARTER') !== false) {
            ...
            $packageName = 'Growth';
            $reviewsQty = '100';
        } elseif (strpos(strtoupper($action), 'ELITE') !== false) {
            $packageName = 'Elite';
            $reviewsQty = '130';
        }
...
foreach ($analyticsData as $event) {
    ...
    if (strpos($eventType, 'ORDER_') === 0 && strpos($eventType, '_CLICK') !== false) {
        ...
        // Track which package this session clicked (handles both 'ORDER_STARTER_CLICK' and 'ORDER_MIDYEAR_STARTER_CLICK')
        if ($eventType === 'ORDER_MIDYEAR_STARTER_CLICK' || $eventType === 'ORDER_STARTER_CLICK') {
            $starterClicks++;
        ...
        if ($eventType === 'ORDER_MIDYEAR_GROWTH_CLICK' || $eventType === 'ORDER_GROWTH_CLICK') {
            $growthClicks++;
            if (!isset($sessionPackageClicks[$sessionId])) {
                $sessionPackageClicks[$sessionId] = [];
            }
        ...
        if ($eventType === 'ORDER_MIDYEAR_ELITE_CLICK' || $eventType === 'ORDER_ELITE_CLICK') {
            $eliteClicks++;
            if (!isset($sessionPackageClicks[$sessionId])) {
                $sessionPackageClicks[$sessionId] = [];
            }
            if (!in_array('elite', $sessionPackageClicks[$sessionId])) {
                $sessionPackageClicks[$sessionId][] = 'elite';
            }
        }
    }
...
foreach ($sessionPackageClicks as $sessionId => $packages) {
    ...
    if (in_array('elite', $packages)) $uniqueSessionsElite++;
}
...
$topPackage = 'N/A';
$topPackageClicks = 0;
if ($starterClicks >= $growthClicks && $starterClicks >= $eliteClicks) {
    $topPackage = 'Starter';
    $topPackageClicks = $starterClicks;
} elseif ($growthClicks >= $eliteClicks) {
    $topPackage = 'Growth';
    $topPackageClicks = $growthClicks;
} else {
    $topPackage = 'Elite';
    $topPackageClicks = $eliteClicks;
}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Smart Buzzer - Mid-Year Sale Dashboard</title>
    <link rel="icon" type="image/x-icon" href="https://smart-buzzer.com/tracker/sb.ico">
    ...
    <div class="header">
        <h1>
            <a href="log.php" style="text-decoration: none; color: inherit; display: inline-flex; align-items: center;">
                <img src="https://smart-buzzer.com/wp-content/uploads/2024/12/REV-COLOR-Smart-Buzzer-11.png" alt="Smart Buzzer" style="height: 32px; vertical-align: middle; margin-right: 10px;">Mid-Year Sale Dashboard
            </a>
        </h1>
                    ...
                    <table class="pricing-table">
                        ...
                        <tbody>
                            ...
                            $packages = [
                                ...
                                'Elite' => ['clicks' => $eliteClicks, 'sessions' => $uniqueSessionsElite]
                            ];
                            foreach ($packages as $name => $data):
                                $pctEngaged = $totalUniqueClickers > 0 ? ($data['sessions'] / $totalUniqueClickers) * 100 : 0;
                                $isHighest = ($data['clicks'] === max($starterClicks, $growthClicks, $eliteClicks));
                            ?>
