...
// === SERVE PROOF-OF-PAYMENT FILE (authenticated only) ===
// proofs/ is HTTP-denied via .htaccess; this streams a proof to a logged-in admin.
if (isset($_GET['proof']) && isset($_SESSION['logged_in'])) {
    $pf = basename($_GET['proof']); // strip any path traversal
    if (!preg_match('/^[A-Za-z0-9._-]+$/', $pf)) {
        http_response_code(400); exit('Invalid file');
    }
    $pfPath = __DIR__ . '/proofs/' . $pf;
    if (!is_file($pfPath)) {
        http_response_code(404); exit('Not found');
    }
    $pfExt = strtolower(pathinfo($pf, PATHINFO_EXTENSION));
    $pfTypes = ['jpg' => 'image/jpeg', 'jpeg' => 'image/jpeg', 'png' => 'image/png', 'webp' => 'image/webp', 'pdf' => 'application/pdf'];
    header('Content-Type: ' . ($pfTypes[$pfExt] ?? 'application/octet-stream'));
    header('Content-Length: ' . filesize($pfPath));
    header('Content-Disposition: inline; filename="' . $pf . '"');
    readfile($pfPath);
    exit;
}
...
// === PARSE SALES DATA (proof-of-payment uploads) ===
// proofs_log.tsv format (8 cols): timestamp|pkg|business|payer_name|email|contact|filename|ip
// legacy (7 cols, pre-email): timestamp|pkg|business|payer_name|contact|filename|ip
$salesFile = __DIR__ . '/proofs_log.tsv';
$sales = [];
if (file_exists($salesFile)) {
    $salesLines = file($salesFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
    $salesLines = array_reverse($salesLines);
    foreach ($salesLines as $sLine) {
        $sp = explode("\t", $sLine);
        if (count($sp) < 6) continue;
        if (count($sp) >= 8) {
            $sale = ['timestamp' => $sp[0], 'pkg' => $sp[1], 'business' => $sp[2], 'name' => $sp[3], 'email' => $sp[4], 'contact' => $sp[5], 'file' => $sp[6]];
        } else {
            // legacy 7-col row (no email)
            $sale = ['timestamp' => $sp[0], 'pkg' => $sp[1], 'business' => $sp[2], 'name' => $sp[3], 'email' => '-', 'contact' => $sp[4], 'file' => $sp[5]];
        }
        $sDate = substr($sale['timestamp'], 0, 10);
...
<body>
    ...
    <div class="tabs">
        ...
        <button class="tab-btn <?php echo $activeTab === 'sales' ? 'active' : ''; ?>" onclick="switchTab('sales')" data-tab="sales">Sales</button>
    ...
    <div class="container">
        ...
        <!-- SALES TAB (Paid — proof of payment uploaded) -->
        <div id="sales-tab" class="tab-content <?php echo $activeTab === 'sales' ? 'active' : ''; ?>">
            <div class="stats-grid">
                <div class="stat-card green">
                    <div class="stat-value"><?php echo number_format($totalSales); ?></div>
                    <div class="stat-label">Paid Orders (Proof Uploaded)</div>
                </div>
            </div>

            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">💰 Sales — Proof of Payment</h3>
                    <span style="color: #5E6C84; font-size: 13px;">Customers who submitted payment proof</span>
                </div>
                <div class="table-wrapper">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($sales as $sale):
                                    $sExt = strtolower(pathinfo($sale['file'], PATHINFO_EXTENSION));
                                    $sIsImg = in_array($sExt, ['jpg', 'jpeg', 'png', 'webp']);
                                    $sProofUrl = '?proof=' . urlencode($sale['file']);
                                ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars($sale['timestamp']); ?></td>
                                        <td><strong><?php echo htmlspecialchars($sale['business']); ?></strong></td>
                                        <td><?php echo htmlspecialchars($sale['name'] !== '' ? $sale['name'] : '-'); ?></td>
                                        <td><?php echo htmlspecialchars($sale['email'] !== '' ? $sale['email'] : '-'); ?></td>
                                        <td><?php echo htmlspecialchars($sale['contact'] !== '' ? $sale['contact'] : '-'); ?></td>
                                        <td><?php echo htmlspecialchars(ucfirst($sale['pkg'])); ?></td>
                                        <td>
                                            <?php if ($sIsImg): ?>
                                                <a href="<?php echo $sProofUrl; ?>" target="_blank" title="Open full proof">
                                                    <img src="<?php echo $sProofUrl; ?>" alt="proof" style="height: 44px; width: auto; border-radius: 6px; border: 1px solid #DFE1E6; vertical-align: middle; margin-right: 8px;">
                                                </a>
                                            <?php endif; ?>
                                            <a href="<?php echo $sProofUrl; ?>" target="_blank" class="btn btn-primary" style="padding: 6px 12px; font-size: 12px;"><?php echo $sExt === 'pdf' ? 'View PDF' : 'View Proof'; ?></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
